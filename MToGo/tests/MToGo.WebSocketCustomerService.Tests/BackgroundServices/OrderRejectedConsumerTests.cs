...
[Collection("Kafka")]
public class OrderRejectedConsumerTests
{
    private readonly SharedKafkaFixture _kafkaFixture;

    public OrderRejectedConsumerTests(SharedKafkaFixture kafkaFixture)
    {
    ...
    [Fact]
    public async Task Consumer_ShouldSendOrderRejected_ToCorrectCustomer()
    {
        // Arrange
        ...
        var consumer = new OrderRejectedConsumer(
            services.BuildServiceProvider(),
            connectionManager,
            new Mock<ILogger<OrderRejectedConsumer>>().Object,
            CreateConfig());
        ...
        var uniqueOrderId = Random.Shared.Next(400000, 499999);
        ...
        await producer.PublishAsync(KafkaTopics.OrderRejected, rejectedEvent.OrderId.ToString(), rejectedEvent);
        ...
        Assert.NotNull(msg);
        Assert.Contains("OrderRejected", msg);
        Assert.Contains("Restaurant is closed", msg);
    }
